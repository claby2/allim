...
static void create_pipe(void) {
    /* Remove PIPE_PATH if it already exists */
    if (access(PIPE_PATH, F_OK) == 0) remove(PIPE_PATH);
    /* Establish FIFO */
    if (mkfifo(PIPE_PATH, S_IWRITE | S_IREAD)) {
        perror("mkfifo");
        die("Could not establish pipe named %s", PIPE_PATH);
    }
}
...
int main(void) {
    ...
    if ((fd = open(PIPE_PATH, O_RDONLY)) < 0) {
        perror("open");
        die("Could not open %s for reading", PIPE_PATH);
    }

    char config_buf[sizeof(struct config)];
    read_checked(fd, config_buf, sizeof(struct config));
    struct config *config = (struct config *)config_buf;

    char *header = str_unmarshal(fd);
    char *text = str_unmarshal(fd);

    printf("%d, %d\n", config->follow_mouse, config->monitor);
    printf("Header: %s\nText: %s\n", header, text);
